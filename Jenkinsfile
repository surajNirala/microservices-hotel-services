pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'snirala1995/hotel_services'
        DOCKER_IMAGE_TAG = "${DOCKER_HUB_REPO}:${env.BUILD_NUMBER}"
        CONTAINER_NAME = 'hotel_services' // Name for your Docker container
        CONTAINER_PORT = '9093' // Port Inside the Docker container
        HOTEL_SERVICE_ENV = credentials('HOTEL_SERVICE_ENV') // For secret file
        SNIRALA_DOCKERHUB_CREDENTIAL = 'snirala-dockerhub-credentials'
        SERVER_1 = '34.131.139.0' 
        CREDENTIALS_SERVER_2 = 'credentials-server-2'
        HOST_PORT = '9093'
        SONARQUBE_KEY = 'sqp_24663b71e254dcfe9ce106702c9aafafc04de830' 
    }
                ...
                script {
                    withSonarQubeEnv('Sonar-Scanner') {
                        sh """
                            ...
                            -Dsonar.projectKey=microservice-hotelservice \
                            -Dsonar.sources=. \
                            -Dsonar.host.url=https://sonarqube.dhimalu.xyz \
                            -Dsonar.login=sqp_24663b71e254dcfe9ce106702c9aafafc04de830
                        """
                    }
                }
            }
